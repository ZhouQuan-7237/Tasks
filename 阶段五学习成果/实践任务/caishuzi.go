...
import (
	"fmt"       // 格式化输入输出
	"math/rand" // 生成随机数
	"time"      // 处理时间相关的操作，如获取当前时间、计算时间间隔和设置随机种子
	"strconv"   // 处理字符串和数值之间的类型转换
	"os"        // 处理操作系统的交互，如读取标准输入、文件操作
	"strings"   // 处理字符串，如去除空格、分割字符串
...
func main() {
	...
	rand.Seed(time.Now().UnixNano())							 // 初始化随机数生成器种子，确保每次运行生成的随机数不同	rand.Seed设置随机数生成器的种子, time.Now()返回当前的时间 ,	UnixNano() 表示当前时间的精确时刻 , time.Now().UnixNano()获取当前时间的纳秒级时间戳 , rand.Seed(time.Now().UnixNano())将当前时间的纳秒级时间戳作为种子传递给随机数生成器,由于每次运行程序时，当前时间都会有所不同，因此这样可以确保每次运行程序时生成的随机数序列都是不同的					
	...
	for {
		...
		if !scanner.Scan() {									//检查是否成功读取了一行用户输入。scanner.Scan() 是 bufio.Scanner 的一个方法，它从输入源（如 os.Stdin）中读取一行内容。如果 scanner.Scan() 成功读取了一行输入（即用户输入了有效内容并按下了回车键），它返回 true;如果读取失败，scanner.Scan() 会返回 false,常见的失败原因包括:输入流结束（比如，用户直接按下回车键导致输入为空行）、输入错误（输入流被意外关闭）
		...
		if err != nil { 										 // 如果转换失败，说明输入不是有效整数。如果 err 为 nil,表示操作成功，没有发生错误;如果 err 不为 nil，表示操作失败，发生了错误。
